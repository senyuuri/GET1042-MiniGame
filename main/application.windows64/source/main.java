...
class Star{
    ...
    public void addNext(String nextstar){
    ...
    public void drawAt(int xx, int yy, int ww, int hh){
        if(this.state == State.UNLOCKED || this.state == State.LOCKED){
...
public void unlockNextStage(Star currStar){
 ...
 * ====================================================== 
 */
public void drawSceneCollection() {
    imageMode(CORNER);
    background(0);
...
public boolean isOverStartButton(){
...
public void drawProgressBar(String name, String unit, int x, int y, int min, int max, int[] value, int c){
    fill(255);
    textSize(15);
    ...
    fill(c);
	int min_len = Math.round(310.0f * (value[0] - min) / (max - min));
	int max_len = Math.round(310.0f * (value[1] - min) / (max - min));
	// draw base bar
...
public void drawColorBar(int ccode, int x, int y){
	fill(255);
    textSize(15);
...
public void drawSceneFactory(){
	background(20, 40, 70);
	
	//machine
	pushMatrix();
	translate(650, 400);
	rotate(radians(speed*.1f*rdir));
	rdir++;
	if (rdir == 2){rdir = -1;}
	...
	if (current != -1){  
		...
		textSize(20);
		if (current == 0){
			value[current] = (float)Math.pow(10, deg[current]/45.0f - 1);
			...
			value[current] = (float)Math.pow(10, deg[current]/20.0f);
			text(nfc(value[current], 1), 940, 680);
			...
			value[current] = deg[current]/180.0f*7;
			if (value[current]<1){
				fill(255);
...
public void mouseClicked() {
    if(currentScene == 0){
		...
		if (bakable && mouseX >= 1110 && mouseX <= 1240 && mouseY >= 450 && mouseY <= 500 && !complete) {
			...
			if (currentStar.ccolor == 0 || currentStar.ccolor == PApplet.parseInt(value[4])+1){success ++;}
			if(success == 5){
				//TBC
...
public void mouseReleased() {
  ...
  public void settings() {  size(displayWidth, displayHeight);  smooth(4); }
